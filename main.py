...
folder_to_run = os.path.join(RUN_DIR, 'run1') # path to folder with videos
all_files = [f for f in os.listdir(folder_to_run)] # list of names of all videos

files = random.sample(all_files, 4) # selects 4 random mp4 files

input_set = []  # list to put in the work function
input_set_len_per_mp4 = []  # keeps track of how many slices each video takes up
...
def process_video(vid, folder):
    ...
    input_set_function = [] # chunk of frames per mp4
    input_set_elements = [] # chunk of frames per 31 iterations
    ...
    while frame_is_present:
        ...
        if not prev_buffer.any():
            raise ValueError("Error checkpoint 1")
        if len(input_set_elements) < 30:
            input_set_elements.append(prev_buffer) # add to chunk
        else:
...
# Part 2, flattening result set and plotting
...
# tfc : total optical flow
# thcRGB : total histogram comparison RGB
# tadc : total absolute difference pixel change
# tgmse : total gray mean squared error
tfc, thcRGB, tadc, tgmse = [], [], [], []

# result set is a 3-D list.
# 3000 frames --> 100 (each slice) by 4(metrics) by 30(computations per slice) array

# semi-flattening 3-D list and creating individual lists for each metric
...
tfc_avg, thcRGB_avg, tadc_avg, tgmse_avg = sum(tfc)/len(tfc), sum(thcRGB)/len(thcRGB), sum(tadc)/len(tadc), sum(tgmse)/len(tgmse) # get average of each list

# divide each element by average (helps with plotting visual)
...
def get_surrounding_lines(arr, line_number, distance, cap):
    """
    Input - arr: metric array, line_number: middle frame, distance: how far out to check, cap: highest possible index (so no out of bounds error)
    Output - metric array b4 line_number, metric[line_number], metric array after line_number
    """
...
def individual_frame_information_helper(scale, largest, threshold):
    """true false threshold checker"""
    if scale > threshold and largest:
        return True
    return False

def individual_frame_information_per_arr_type(arr_type, line_num, reach):
    """will check if current 'line_num' is a scene change or not per each metric"""
...
def individual_frame_information(line_num, reach):
    """will check if current 'line_num' is a scene change or not"""
    boolean_count = []
...
def create_barriers(line_num_start, line_num_end):
    """create list of possible scene changes"""
    barriers = []
...
def create_every_dictionary(total_pixel_change_arr, window):
    """for every video, brute force create every possible scene change"""
...
def final_filter_dictionary(dictionary_yay, barriers, desired_options):
    dictionary_ordered = sorted(dictionary_yay.items(), key=lambda x: x[1], reverse=True) # dictionary of every frame, sorted by highest average total pixel change
    final = [] 
    iteration_count = 0
    for key, value in dictionary_ordered: # key : (start frame, end frame), value : average total pixel change
        iteration_count += 1
        left, right = key[0], key[1]
        interfere = False 
        for pair in final: # check so we have no overlapping scenes
            pair_left, pair_right = pair[0][0], pair[0][1]
            if pair_left <= left <= pair_right or pair_left <= right <= pair_right: 
                interfere = True
                break
        if not interfere:  # check so scenes in general don't overlap with scene changes
            for blockage in barriers:
                if left <= blockage <= right:
                    interfere = True
                    break
        if interfere: continue 
        final.append((key, value, iteration_count)) # append if scene passes both checks
        if len(final) > desired_options:
            return final
    return final


# figure out how many frames each video has
total_frames_each_video_has = []
for test_video in files: # iterate through videos
    source_path = os.path.join(RUN_DIR, 'run1', test_video) # source path
    clipclip = cv2.VideoCapture(source_path) # capture video
    total_frames = int(clipclip.get(cv2.CAP_PROP_FRAME_COUNT)) - 1 # get total frames
    clipclip.release()
    print(total_frames)
    total_frames_each_video_has.append(total_frames) # append frames to list


# creating the amv
start_index = 0

video_list = [] # getting best frames

video_list_tier_2 = [] # getting second best frames

for i in range(len(files)):
    print(files[i], i) # files[i]: name of file

    # slice the arrays computed by the work function for the current iterated video
    end_index = start_index + total_frames_each_video_has[i]
    ...
    # find possible cut scenes and append those indices to list 'barbar' (works like 60-70% of the time)
    barbar = create_barriers(50, total_frames_each_video_has[i])
    print(f"{files[i]}: ",barbar)

    # create every possible subclip
    dictdict = create_every_dictionary(curr_tadc, 72)  # 72 represents size of subclip. If vid is 24 fps, 72 is 3 seconds of content

    # filter out subclips that wouldn't be ideal for an AMV
    # finalfinal holds frames representing subclips that may work for current video file
    finalfinal = final_filter_dictionary(dictdict, barbar, 3) # 3 is how many possible subclips to return
    print(finalfinal)

    # update the starting index for the next iteration
    start_index += total_frames_each_video_has[i]

    # get source path to current iteration
    source_path = os.path.join(RUN_DIR, 'run1', files[i])

    # read the current file and get frmes per second
    clip = VideoFileClip(source_path)
    fps = clip.reader.fps

    # since moviepy and cv2 subclips videos by time and not frames, converting frames to time
    # if final_filter_dictionary found a good scene, scene is used
    # if final_filter_dictionary found a low action scene or returned an empty list, recalculates without barriers
    # lol code below pretty unreadable
    found = False
    if finalfinal:
        clippyclippy = finalfinal[0]
        if clippyclippy[1] < 100: # if scene has minimal action (100 is a threshold value, changes depending on video medium)
            pass
        else:
            found = True
            filefilefile = files[i] # mp4 file name
            startstartstart = round(finalfinal[0][0][0]/fps, 3) # start time
            endendend = round(finalfinal[0][0][1]/fps, 3) # endtime
    ...
    if len(finalfinal) > 1:
        ...
        video_list_tier_2.append(tupletupletuple2)
        found2 = True
    if found2:
        print("so far small: ", video_list_tier_2)
        continue
    else:
        temp_bar_bar_2 = []
        ...
        video_list_tier_2.append(temp_tupletupletuple_2)


print("final clips")
print(video_list)
print(video_list_tier_2)


video_clips = [] # list of subclips
video_clips_second_tier = [] # list of tier 2 subclips

audio_clip_path_4 = os.path.join(BASE_DIR, "test_audio_1.mp3") # 12 second amv, best scene from each video file
...
for video_file, start_time, end_time in video_list: # create subclips and append to video_clips
    source_path = os.path.join(RUN_DIR, 'run1', video_file)
    video = VideoFileClip(source_path).subclip(start_time, end_time)
    video_clips.append(video)

for video_file_short, start_time_short, end_time_short in video_list_tier_2: # create subclips and append to video_clips_second_tier
    ...
    video_clips_second_tier.append(video_short)


import random
amv_output_path = os.path.join(RUN_DIR, 'run1_output') # amv output path


random.shuffle(video_clips)
random.shuffle(video_clips_second_tier)

print("tier 1", video_clips)
print("tier 2", video_clips_second_tier)
...
final_clip_4 = concatenate_videoclips(final_video_4) # concatenate clips
print(final_clip_4.fps)
print("------")
print(audio_clip_4.fps)
audio_clip_4 = audio_clip_4.set_fps(final_clip_4.fps) # set audio to same fps as video
final_clip_4 = final_clip_4.set_audio(audio_clip_4) # add audio to final video
output_path_4 = os.path.join(amv_output_path, "final_video_4.mp4") # output path
final_clip_4.write_videofile(output_path_4, codec='libx264', audio_codec='aac', audio=True) # save video locally

# 24 sec amv
final_video_clips = [] 

for i2 in range(len(video_clips_second_tier)):
    final_video_clips.append(video_clips_second_tier[i2])

final_video_clips.extend(final_video_4) # first 4 subclips are tier 2, next 4 are tier 1


print("aaa", final_video_clips)
final_clip = concatenate_videoclips(final_video_clips) # concatenate clips
...
output_path_8 = os.path.join(amv_output_path, "final_video_8.mp4") # output path
final_clip.write_videofile(output_path_8, codec='libx264', audio_codec='aac', audio=True) 
